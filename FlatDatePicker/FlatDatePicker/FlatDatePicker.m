...
- (void)buildSelectorLabelsMonths {
    ...
    NSArray *cidades = @[@"Rio de Janeiro", @"Búzios", @"Angra dos Reis", @"Paraty", @"Petrópolis", @"Tiradentes", @"São Paulo"];
    int count = 0;
    ...
    for (int i = 0; i < _months.count; i++) {
        ...
        if (count < cidades.count) {
            labelDay.text = [cidades objectAtIndex:count];
        }
        else
        {
            labelDay.text = [cidades objectAtIndex:0];
            count = 0;
        }
        count++;
